...
def forward_selection(X_train, y_train):
    sfs = SequentialFeatureSelector(LinearRegression(), direction='forward', scoring='f1', cv=None)
...
def backward_selection(X_train, y_train):
    sbs = SequentialFeatureSelector(LinearRegression(), direction='backward', scoring='f1', cv=None)
